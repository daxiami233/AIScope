...
### OpenCode Go
- **自动检测**：安装 OpenCode 并连接 OpenCode Go 后自动检测
- **首次使用**：在「工具配置」中点击“登录官网”，完成 OpenCode 官方登录
- **额度来源**：官网登录会话读取工作区实时 5h、7d、月度额度及官方刷新时间
- **凭证保存**：官网登录 Cookie 保存在 macOS Keychain；不会写入 OpenCode 的本地配置
- **兜底机制**：官网会话失效、离线或页面接口变化时，使用 `~/.local/share/opencode/opencode.db` 的历史记录估算，并在详情中标记为“本机历史估算”
